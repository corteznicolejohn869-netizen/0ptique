...
const THEME_STORAGE_KEY = 'websiteTheme'; // NEW: Key for storing theme preference
...
// HOME SLIDES (Assuming this logic is correct for index.html)
...
const updateThemeIcon = (isLightMode) => {
    ...
    if (themeBtn) {
        if (isLightMode) {
            ...
            themeBtn.classList.add('fa-moon'); // Show moon icon to suggest switching to dark
        } else {
            themeBtn.classList.remove('fa-moon');
            themeBtn.classList.add('fa-sun'); // Show sun icon to suggest switching to light
        }
    }
};
...
const renderOrderSummary = () => {
    ...
    if (cart.length === 0) {
        container.innerHTML = '<p style="text-align:center; padding: 1rem; color: var(--gray);">Your cart is empty. Please return to the products page.</p>';
        updateCartTotal(); 
        return;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Set initial theme if it doesn't exist (default is dark)
    ...
    // NEW: Apply the saved theme preference on load
    applySavedTheme(); 
